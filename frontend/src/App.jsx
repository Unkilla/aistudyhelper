...
function App() {
  ...
  async function generate(toolName, file = noteFile) {
    ...
    try {
      ...
      // Uploads straight to Vercel Blob from the browser — never passes through
      // our backend, so there's no serverless body-size limit to hit.
      const blob = await upload(file.name, file, {
        access: 'public',
        handleUploadUrl: `${backendDomain}/api/upload`,
      })
      const response = await fetch(`${backendDomain}/api/generate`, {
        method: 'POST',
